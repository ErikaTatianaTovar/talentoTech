...
describe('PATCH /housing/:code', () => {
    it('responds with a specific object update', async () => {
        ...
        const response = await request(app).patch('/housing/' + createdHousingCode).send(UpdateObjectToTest)
    ...
    it('error update specific object', async () => {
        ...
        const response = await request(app).patch('/housing/' + InvalidUpdateObject).send(UpdateObjectToTest)
        expect(response.status).toBe(500);
    })
})
...
describe('DELETE /housing/:code', () => {
    it('responds with a specific object delete', async () => {
        const response = await request(app).delete('/housing/' + createdHousingCode);
        
        expect(response.status).toBe(200);
        expect(typeof response.body === "object").toBe(true)

        const deletedHousing = await HousingSchema.findOne({ code: createdHousingCode });
